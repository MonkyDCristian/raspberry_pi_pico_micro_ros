/*
    ...
    ros2 topic pub --once /wamv_caleuche/thrusters/left/turn_on_off std_msgs/msg/Bool data:\ true    -> turn off the motor
...
unsigned long last_cmd_time;  // Time variable for the timeout
int timeout = 0;  // Timeout flag
int first_msg  = 0;  // Motor on flag
bool turn_on = false;  // Motor on flag
...
void loop()
{
    ...
    if (timeout == 0 and first_msg == 1 and (micros() - last_cmd_time) > (2 * 1000000)) {
        timeout = 1;
    }
...
bool microros_create_entities(){
    ...
    // Node information
    const char *node_name = "thruster_left";
    ...
    RCCHECK(rclc_subscription_init_default( // topic for turning on and off the thruster
        ...
        "/wamv_caleuche/thrusters/left/turn_on_off"));
    
    RCCHECK(rclc_subscription_init_default( // topic for giving velocity commands to the thruster
        &cmd_vel_sub, 
        &node,
        ROSIDL_GET_MSG_TYPE_SUPPORT(std_msgs, msg, Int16), 
        "/wamv_caleuche/thrusters/left/cmd_vel"));

    // ---- MICROROS SERVICES ----
    const char * service_name = "/wamv_caleuche/thrusters/left/feedback";
...
void sub_turn_callback(const void * msgin){
    ...
    if (!turn_on) {
        digitalWrite(LED_BUILTIN, HIGH);  // If there is an error, the LED will turn on
        turn_on = true;
    }
    else {
        digitalWrite(LED_BUILTIN, LOW);  // If there is an error, the LED will turn on
...
void service_callback(const void * req_msg, void * res_msg){
    ...
    double rpm_f, battery_f, error_f, rpm_sign;

    if (turn_on){
        battery_f = 100.0;
        if (!timeout){
            rpm_f = (double)cmd_vel;
            error_f = 0.0;
        } else {
            rpm_f = 0.0;
            error_f = 50.0;
